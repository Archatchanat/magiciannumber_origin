...
<?
	if($_GET["action"]=="insert")
	{
		...
		// generate auto
			...
			$tel1  = "select * from job_category where cat_number1 = '".$cat_number1."'";
			$get1  = mysql_query($tel1);
			$have1 = mysql_num_rows($get1);
		
			$tel2  = "select * from job_category where cat_number2 = '".$cat_number1."'";
			$get2  = mysql_query($tel2);
			$have2 = mysql_num_rows($get2);
		
			$tel3  = "select * from job_category where cat_number1 = '".$cat_number2."'";
			$get3  = mysql_query($tel3);
			$have3 = mysql_num_rows($get3);
		
			$tel4  = "select * from job_category where cat_number2 = '".$cat_number2."'";
		...
		//------------------------------------------------------------------------------------------------------------
		$tellway  = "INSERT INTO job_category VALUES(";
		$tellway .= "0
					,'$_POST[cat_title]'
					,'$cat_number1'
					,'$cat_number2'
		...
		echo "window.location='job_add_category.php';";
		echo "</script>";
    }
	...
	if($_GET["action"]=="save")
	{
		...
		$sql_up = "update job_category set 
	...
	if($_GET["del"]=="true")
	{
		$sql_del= "delete from job_category where cat_id='".$_GET["cat_id"]."'";
...
function checkvalue(){
	if(form1.cat_title.value == "") {
		alert("โปรดระบุ ชื่อหมวดเบอร์อาชีพ");
    ...
    <div class="main">
			...
			<tr> 
				<td colspan="2" background="images/bghead.png"align="left">
					<font color="#FFFFFF" size="+0">&nbsp;เพิ่มหมวดหมู่อาชีพ</font>
				</td>
			</tr>
			...
			<tr>
				<td width="25%" align="right">ชื่อหมวดหมู่อาชีพ :</td>
		  ...
		  <!--------------------------------------------------------------------------------------------------------------- -->
			...
			<tr> 
				<td colspan="2" align="left" bgcolor="#999999">
					<font color="#FFFFFF" size="+0">&nbsp;แก้ไขหมวดหมู่อาชีพ</font>				
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					...
					&nbsp;&nbsp;<a href="job_add_category.php">[ ย้อนกลับ ]</a>
				</td>
			</tr>
		  ...
		  <table width="90%" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF">
			...
			<tr> 
				<td colspan="5" background="images/bghead.png"align="left">
					<font color="#FFFFFF" size="+1">&nbsp;รายการหมวดหมู่อาชีพ</font>				
				</td>
			</tr>
			...
			<tr bgcolor="#CCCCCC">
				...
				<td align="center">ชื่อหมวดหมู่อาชีพ</td>
			...
			<?
				$tel_cat ="select * from job_category order by cat_title ASC";
				$get_cat = mysql_query($tel_cat);
